...
import ImageItems from './Components/Image Items'
import './App.css'
...
class App extends Component {
  ...
  getShuffledImagesList = () => {
    const {initialList} = this.state
    const randomList = initialList.sort(() => Math.random() - 0.5)

    return randomList
  }

  render() {
    const {score, seconds, activeTabId} = this.state
    const randomList = this.getShuffledImagesList()
    const filterImagesList = this.getFilteredImages()
    return (
      <div className="app-container">
        <Navbar score={score} seconds={seconds} />
        <div className="bg-container">
          <div className="random-image-container">
            <DisplayImage displayImage={randomList} />
          </div>
          ...
          <ul className="project-list-container">
            {filterImagesList.map(imageDetails => (
              <ImageItems key={imageDetails.id} imageDetails={imageDetails} />
            ))}
          </ul>
        </div>
      </div>
